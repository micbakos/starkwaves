...
fn test_new_game_6x6() {
    let game = GameTrait::new(1, player_a(), player_b(), 6);
    assert!(game.id == 1, "Game ID should be 1");
    assert!(game.board_size == 6, "Board size should be 6");
...
fn test_new_game_8x8() {
    let game = GameTrait::new(2, player_a(), player_b(), 8);
    assert!(game.board_size == 8, "Board size should be 8");
}

#[test]
fn test_new_game_10x10() {
    let game = GameTrait::new(3, player_a(), player_b(), 10);
    assert!(game.board_size == 10, "Board size should be 10");
}

#[test]
fn test_new_game_12x12() {
    let game = GameTrait::new(4, player_a(), player_b(), 12);
    assert!(game.board_size == 12, "Board size should be 12");
}

#[test]
fn test_new_game_14x14() {
    let game = GameTrait::new(5, player_a(), player_b(), 14);
    assert!(game.board_size == 14, "Board size should be 14");
}

#[test]
fn test_new_game_20x20() {
    let game = GameTrait::new(6, player_a(), player_b(), 20);
    assert!(game.board_size == 20, "Board size should be 20");
}

#[test]
#[should_panic(expected: "Board is not a valid size.")]
fn test_new_game_invalid_size_5() {
    GameTrait::new(1, player_a(), player_b(), 5);
}

#[test]
#[should_panic(expected: "Board is not a valid size.")]
fn test_new_game_invalid_size_7() {
    GameTrait::new(1, player_a(), player_b(), 7);
}

#[test]
#[should_panic(expected: "Board is not a valid size.")]
fn test_new_game_invalid_size_15() {
    GameTrait::new(1, player_a(), player_b(), 15);
}
...
fn test_commit_root_player_a() {
    let mut game = GameTrait::new(1, player_a(), player_b(), 6);
...
fn test_offset_bytes_large_board() {
    let game = GameTrait::new(1, player_a(), player_b(), 20);
...
fn test_offset_roundtrip() {
    ...
    while i < coords.len() {
        ...
        let board_size: u32 = game.board_size.into();
        let reconstructed_x: u8 = (offset / board_size).try_into().unwrap();
        let reconstructed_y: u8 = (offset % board_size).try_into().unwrap();
