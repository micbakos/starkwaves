...
use crate::types::{AppState, ScreenKind, ScreenState};
...
impl NavCommand {
    ...
    pub fn handle(&self, state: &mut AppState) {
        match self {
            ...
            NavCommand::PopTo(screen_kind) => {
                let index = state.screens.iter().position(|screen_state| {
                    screen_state.kind() == *screen_kind
                }).unwrap_or_else(|| panic!("Expected to pop to {}, but such screen doesn't exist.", screen_kind));
