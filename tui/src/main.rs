...
async fn run(mut terminal: DefaultTerminal) -> Result<()> {
    ...
    let (state_sender, state_receiver) = watch::channel(AppState::start());
    ...
    tokio::spawn(async move {
        while let Some(effects) = effects_receiver.recv().await {
            for effect in effects {
                ...
                tokio::spawn(AppScreen::run(effect, intent_sender));
            }
        }
    });
        ...
        loop {
            if let Ok(Event::Key(event)) = crossterm::event::read() {
                ...
                let intent = match top_screen {
                    ...
                    ScreenState::Start(_) => {
                        start::screen::StartScreen::on_key(event).map(|i| i.into())
                    }
                    ScreenState::Login(_) => {
                        login::screen::LoginScreen::on_key(event).map(|i| i.into())
                    }
                }
                .or_else(|| AppScreen::on_key(event));
